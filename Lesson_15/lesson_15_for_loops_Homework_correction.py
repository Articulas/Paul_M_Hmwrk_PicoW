...
print ('=-=-=-=-=-=-=-=-=-=-=-=-=-=')
numColors = int (input('Number of Colors: ')) # if you do the input as an int you dont have to change it later. 
print ('=-=-=-=-=-=-=-=-=-=-=-=-=-=')
# Step 2 Get each individual color input
print ('=-=-=-=-=-=-=-=-=-=-=-=-=-=')
for i in range (0,numColors,1): # 0 is start of for loop, current cycle, 1 is increment change
    chColors = input ('Color choice #' + str(i + 1) + ' ')
    chColors = chColors.lower() # Converts upper case to lower case
